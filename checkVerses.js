...
// جدول عدد الآيات الصحيح لكل سورة (من 1 إلى 114)
const ayaCounts = {
...
try {
  const rawData = fs.readFileSync("quran.json", "utf8");
  const quran = JSON.parse(rawData);

  console.log("🔍 التحقق من عدد آيات كل سورة:\n");

  for (const [suraStr, verses] of Object.entries(quran)) {
    const suraNum = parseInt(suraStr, 10);
    const actual = verses.length;
    const expected = ayaCounts[suraNum];

    if (expected === undefined) {
      console.log(`⚠️ سورة ${suraNum}: غير موجودة في جدول الأعداد!`);
    } else if (actual === expected) {
      console.log(`✅ ${suraNum} : (${actual})`);
    } else {
      console.log(`❌ ${suraNum}:${expected}، ${actual}`);
    }
  }
